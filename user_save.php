...
if (empty($password) || empty($login) || empty($password_repeat) || empty($student_group)
    or empty($city)) {
...
if(!empty($found_user)) {
    ...
    $new_result = $mysqli->prepare("INSERT INTO users (login_email, password, city, student_group, date_created, status, role)
                      VALUES (?,?,?,?,?,?,?)");
    $new_result->bind_param("sssssss", $login, $password, $city, $student_group, $curdate, $status, $role);
